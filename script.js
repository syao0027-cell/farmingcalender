...
const supabaseUrl = https://tscogqlhlavulqgncodj.supabase.co/rest/v1/
    ...
    calendar = new FullCalendar.Calendar(
        document.getElementById('calendar-view'),
        {
            ...
            eventDidMount: (info) => {
                ...
                const cropName =
                    info.event.extendedProps.crop || "その他";

                const cropData =
                    crops.find(c => c.name === cropName);

                info.el.style.backgroundColor =
                    cropData ? cropData.color : "#bdbdbd";
...
function updateCropSelect() {
    ...
    const select =
        document.getElementById('task-crop');
...
function updateMemberSelect() {
    ...
    const select =
        document.getElementById('task-member');
...
function openEditForm(event) {
    ...
    document.getElementById('task-crop').value =
        event.extendedProps.crop || "その他";

    document.getElementById('task-member').value =
        event.extendedProps.member || "";
...
function updateTodayTasks(tasks) {
    ...
    tasks
        ...
        .forEach(t => {
            ...
            const crop =
                t.extendedProps.crop || "その他";

            const member =
                t.extendedProps.member || "";

            const area =
                t.extendedProps.area || "";

            const memoText =
                t.extendedProps.memo
                    ? `<br><small style="color:#795548;">
                        📝 ${t.extendedProps.memo}
                       </small>`
                    : "";
